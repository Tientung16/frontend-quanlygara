...
function RepairCarUpdate({ record,handleCloseUpdate }: Props) {
  ...
  const { GetDataSelectAutomaker, ListComboboxAutomaker,} = AutomakerHooks();
    ...
    const loadData = async (record:any) => {
        ...
        console.log('EntityV2',EntityV2);
        console.log('Entity',Entity);
        console.log('EntityRepairCar',EntityRepairCar);

        // setDataSource(dataConvert)
        setDataSource(c.data)
    }
    ...
    const handleChangeData = (indexChange:any, nameChange:any, valueChange:any) => {
      ...
      if(nameChange === 'number'){
        const filteredList = listComboboxAccessory.filter((item:any) => dataClone.some((dataItem:any) => dataItem.idAccessory === item.id));
        const filteredList2 = dataClone.filter((item:any) => listComboboxAccessory.some((dataItem:any) => dataItem.id === item.idAccessory));
        ...
        total = total + parseInt(wage);
        formModal.setFieldValue('totalMoney', total);
    ...
    const columns: TableColumnsType<any> = [
      ...
      },
      {
          ...
          render(number,record,index) {
            formModal.setFieldValue(`number-${index}`, number);
              return (
              <>
  ...
  const handleTotalMoney = (value:any) => {
    console.log('wage',wage);
    const total = parseInt(totalMoney) - parseInt(wage) + parseInt(value);
    formModal.setFieldValue('totalMoney', total);
  ...
  const onFinish = (values:any) => {
    ...
    // const filteredList2 = dataSource.filter((item:any) => listComboboxAccessory.some((dataItem:any) => dataItem.id === item.idAccessory));
    ...
    UpdateRepairCar(dataUpdateReceiveCar);
  };
    ...
    <>
      <Form layout="vertical" form={formModal} onFinish={onFinish}>
        ...
        <Row gutter={24}>
          ...
          <Col span={12}>
            <Form.Item name="totalMoney" label="Tổng tiền"
            rules={[
              {
                required: true,
                message:  "không được để trống",
...
export default RepairCarUpdate;