...
const apiGetAllCustomer = `${apiUrl}/api/accessory-getAll`;
const apiCreateCustomer = `${apiUrl}/api/accessory`;
const apiGetDetail = `${apiUrl}/api/accessory`;
const apiUpdateCustomer = `${apiUrl}/api/accessory`;
const apiDeleteCustomer = `${apiUrl}/api/accessory`;
const apiCombobox = `${apiUrl}/api/List-accessory`;
const apiAccessoryTotalpage = `${apiUrl}/api/accessory-page-number`;
...
//   });
export const getEntitiesAccessory = createAsyncThunk('Accessory/fetch_entity_list_search', async ({ query }: any) => {
  const requestUrl = `${apiGetAllCustomer}?${query}&cacheBuster=${new Date().getTime()}`;
  return axios.post<any>(requestUrl);
});

export const getDataSelectAccessory = createAsyncThunk(
  'CommonSlice/fetch_categoryType_ConfigGuidea',
  async () => {
...
async () => {
  const requestUrl = `${apiAccessoryTotalpage}`;
  return axios.get<any>(requestUrl);
};
export const createEntity = createAsyncThunk(
  'Accessory/create_entity',
...
export const AccessorySlice = createEntitySlice({
  name: 'Accessory',
  initialState,
//   reducers: {
...
//   },
  extraReducers(builder) {
    builder
      ...
      // })
      .addCase(getDataSelectAccessory.fulfilled, (state, action) => {
        const { data } = action.payload;
        state.loading = false;
        const dataMapping = MappingDTO(data, 'id', 'name');
        state.listComboboxAccessory = dataMapping;
      })
      .addMatcher(isFulfilled(getEntitiesAccessory), (state, action) => {
      ...
      .addMatcher(isPending( getEntitiesAccessory), state => {
        state.errorMessage = null;
        state.updateSuccess = false;
        state.loading = true;
      });
...
export const { reset } = AccessorySlice.actions;
