...
const apiGetAllCustomer = `${apiUrl}/api/automaker-getAll`;
const apiCreateCustomer = `${apiUrl}/api/automaker`;
const apiGetDetail = `${apiUrl}/api/automaker`;
const apiUpdateCustomer = `${apiUrl}/api/automaker`;
const apiListAutomaker = `${apiUrl}/api/List-automaker`;
const apiDeleteCustomer = `${apiUrl}/api/automaker`;
const apiAutomakerTotalPage = `${apiUrl}/api/automaker-page-number`;
...
//   });
export const getEntitiesAutomaker = createAsyncThunk('Automaker/fetch_entity_list_search', async ({ query }: any) => {
  const requestUrl = `${apiGetAllCustomer}?${query}&cacheBuster=${new Date().getTime()}`;
  return axios.post<any>(requestUrl);
});
...
export const getEntity = 
    ...
    export const getDataSelectAutomaker = createAsyncThunk(
      'CommonSlice/fetch_categoryType_ConfigGuide',
      async () => {
        const requestUrl = `${apiListAutomaker}`;
        return axios.post<any>(requestUrl);
      },
...
async () => {
  const requestUrl = `${apiAutomakerTotalPage}`;
  return axios.get<any>(requestUrl);
};
export const createEntity = createAsyncThunk(
  'Automaker/create_entity',
...
export const AutomakerSlice = createEntitySlice({
  name: 'Automaker',
  initialState,
//   reducers: {
...
//   },
  extraReducers(builder) {
    builder
      ...
      // })
      .addCase(getDataSelectAutomaker.fulfilled, (state, action) => {
        const { data } = action.payload;
        state.loading = false;
        const dataMapping = MappingDTO(data, 'id', 'name');
        state.listComboboxSupplier = dataMapping;
      })
      .addMatcher(isFulfilled(getEntitiesAutomaker), (state, action) => {
      ...
      .addMatcher(isPending( getEntitiesAutomaker), state => {
        state.errorMessage = null;
        state.updateSuccess = false;
        state.loading = true;
      });
...
export const { reset } = AutomakerSlice.actions;
